...
def build_hybrid_model(n_features=60, n_steps=4, n_temporal_features=24, n_classes=10):
    # Amplitude branch (MLP)
    amp_input = keras.Input(shape=(n_features,), name='amplitude_features')
    x1 = layers.Dense(128, activation='relu')(amp_input)
    x1 = layers.Dropout(0.2)(x1)
    x1 = layers.Dense(64, activation='relu')(x1)
    amp_out = layers.Dense(32, activation='relu')(x1)

    # Temporal branch (GRU)
    temp_input = keras.Input(shape=(n_steps, n_temporal_features), name='temporal_features')
    x2 = layers.GRU(32, return_sequences=False)(temp_input)
    x2 = layers.Dense(32, activation='relu')(x2)
    temp_out = layers.Dropout(0.2)(x2)
